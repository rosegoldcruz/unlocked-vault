...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let query = getSupabaseAdmin()
      .from('iv_payout_jobs')
      .select('id, privy_user_id, milestone_number, wallet_address, token_mint, amount_raw, status, attempts, max_attempts, next_attempt_at, last_error, locked_at, locked_by, created_at, updated_at')
    ...
    const message = error instanceof Error ? error.message : 'Failed to load payout jobs'
