        ...
        <form onSubmit={submitTicket} className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-6 space-y-4">
          ...
          {(['name', 'email', 'subject'] as const).map((field) => (
            <div key={field}>
              ...
              <input id={`ticket-${field}`} value={form[field]} onChange={(e) => setForm((prev) => ({ ...prev, [field]: e.target.value }))} className="w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100" required={field === 'subject'} />
            </div>
          ))}
          <div>
            <label htmlFor="ticket-message" className="mb-1.5 block text-sm text-zinc-300">Message</label>
            <textarea id="ticket-message" value={form.message} onChange={(e) => setForm((prev) => ({ ...prev, message: e.target.value }))} className="min-h-32 w-full rounded-md border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm text-zinc-100" required />
          </div>
